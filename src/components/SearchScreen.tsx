  ...
  return (
    <div className="flex-grow flex flex-col p-4 md:p-8 max-w-7xl mx-auto w-full gap-6">
      ...
      <section id="hero-search-section" className="flex flex-col gap-2 pt-2 md:pt-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="font-body-lg text-body-lg text-[#434654] text-sm md:text-base">
              Enter 5-digit bus stop code (e.g. <strong>83139</strong>, <strong>01012</strong>, <strong>09048</strong>), road name, or service number.
            </p>
          </div>
        ...
        <div className="relative w-full mt-3">
          ...
          <input
            ...
            placeholder="Search stop code (e.g. 83139), Marine Parade, Orchard, Bugis, Service 15..."
            ...
            <div className="absolute top-full left-0 right-0 mt-2 bg-[#ffffff] border border-[#c3c6d6] rounded-2xl shadow-xl z-30 max-h-80 overflow-y-auto divide-y divide-[#ededf8]">
              ...
              {isCustomStopCode && !existingStopForCode && (
                ...
                >
                  ...
                  <span className="font-label-caps text-xs bg-[#003d9b] text-[#ffffff] px-2 py-0.5 rounded">
                    Query Stop
                  </span>
                </div>
              )}
                ...
                <ul className="py-1">
                  {filteredStops.map((stop) => (
                    <li
                      ...
                      className="px-4 py-3 hover:bg-[#f3f3fd] cursor-pointer flex items-center justify-between transition-colors"
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="font-body-md font-semibold text-[#191b23]">{stop.name}</p>
                          <span className="font-label-caps text-xs bg-[#ededf8] text-[#003d9b] px-1.5 py-0.5 rounded font-bold">
                            {stop.id}
                          </span>
                        </div>
                        <p className="font-label-caps text-[#434654] text-xs mt-0.5">
                          {stop.intersection}
                        </p>
                      </div>
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {stop.routes.slice(0, 4).map((r) => (
                          <span
                            key={r}
                            className="bg-[#003d9b] text-[#ffffff] font-label-caps text-xs px-2 py-0.5 rounded"
                          >
                            {r}
                          </span>
                        ))}
                        {stop.routes.length > 4 && (
                          <span className="text-[10px] text-[#737685] font-label-caps">
                            +{stop.routes.length - 4} more
                          </span>
                        )}
                        <span className="material-symbols-outlined text-[#737685]">arrow_forward</span>
                      </div>
                    </li>
                  ))}
                </ul>
                ...
                !isCustomStopCode && (
                  <div className="p-5 text-center text-[#737685] font-body-md">
                    ...
                    <p className="text-xs text-[#515f74] mt-1">
                      Tip: Type any 5-digit bus stop code (e.g. 83139) to fetch live LTA arrivals.
                    </p>
                  </div>
                )
              )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 flex-grow mt-1">
        {/* Left Column: Recent & Suggestions (4 cols) */}
        <div className="md:col-span-4 flex flex-col gap-5">
              ...
              <ul className="flex flex-col gap-2">
                {savedStops.map((stop) => (
                  <li
                    key={stop.id}
                    onClick={() => handleStopClick(stop)}
                    className="flex items-center gap-2 p-2.5 hover:bg-[#f3f3fd] rounded-xl cursor-pointer group transition-colors border border-transparent hover:border-[#dae2ff]"
                  >
                    <span className="font-label-caps text-xs bg-[#003d9b] text-[#ffffff] px-2 py-1 rounded font-bold">
                      {stop.id}
                    </span>
                    <div className="flex-grow min-w-0">
                      <p className="font-body-md text-[#191b23] font-medium text-sm truncate">{stop.name}</p>
                      <p className="font-label-caps text-[#434654] text-[11px] truncate">{stop.roadName || stop.intersection}</p>
                    </div>
          ...
          {/* Key Singapore Bus Stops */}
          <div
            id="card-recent-searches"
            className="bg-[#ffffff] border border-[#c3c6d6] rounded-2xl p-4 shadow-xs"
          >
            <h2 className="font-label-caps text-xs text-[#515f74] mb-3 font-bold tracking-wider">
              POPULAR SINGAPORE STOPS
            </h2>
            <ul className="flex flex-col gap-2">
              {TRANSIT_STOPS.slice(0, 4).map((stop) => (
                <li
                  key={stop.id}
                  onClick={() => handleStopClick(stop)}
                  className="flex items-center gap-3 p-2 hover:bg-[#f3f3fd] rounded-xl cursor-pointer group transition-colors border border-transparent hover:border-[#dae2ff]"
                >
                  <span className="font-label-caps text-xs bg-[#ededf8] group-hover:bg-[#003d9b] text-[#003d9b] group-hover:text-[#ffffff] px-2 py-1 rounded font-bold transition-colors">
                    {stop.id}
                  </span>
                  <div className="flex-grow min-w-0">
                    <p className="font-body-md text-sm text-[#191b23] font-medium group-hover:text-[#003d9b] transition-colors truncate">
                      {stop.name}
                    </p>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              {selectedRouteFilter && (
                <button
                  ...
                  className="font-label-caps text-xs text-[#003d9b] hover:underline cursor-pointer"
                >
                  Clear Filter
                </button>
              )}
        ...
        {/* Right Column: Map / Nearby (8 cols) */}
        <div
          id="container-transit-map"
          className="md:col-span-8 flex flex-col h-full min-h-[460px] border border-[#c3c6d6] rounded-2xl overflow-hidden relative group bg-[#ededf8] shadow-xs select-none"
        >
          {/* Map Base Image */}
          <img
            src={MAP_IMAGE_URL}
            ...
            {TRANSIT_STOPS.map((stop) => {
              ...
              return (
                <div
                  ...
                  title={`${stop.name} (Stop ID: ${stop.id})`}
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center border-2 border-[#ffffff] shadow-lg transition-all ${
                      isSelected
                        ? 'bg-[#001848] ring-4 ring-[#0052cc]/50 scale-110'
                        : 'bg-[#003d9b] hover:bg-[#0052cc]'
                    }`}
                  >
                    <span className="material-symbols-outlined text-[#ffffff] text-[16px]">
                      directions_bus
                    </span>
                  </div>
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 bg-[#ffffff]/95 backdrop-blur-sm border border-[#c3c6d6] px-1.5 py-0.5 rounded shadow-sm whitespace-nowrap text-[10px] font-label-caps text-[#191b23] pointer-events-none hidden md:block font-bold">
                    {stop.id}
                  </div>
                </div>
              );
          ...
          {/* Interactive Map Callout Popup */}
          {activeMapStop && (
            <div
              id="map-callout-popup"
              className="absolute top-4 right-4 md:right-8 z-30 w-72 md:w-80 bg-[#ffffff]/95 backdrop-blur-md border border-[#c3c6d6] rounded-2xl p-4 shadow-xl transition-all animate-fadeIn"
            >
              <div className="flex justify-between items-start mb-2">
                <div>
                  <div className="flex items-center gap-1.5 mb-1">
                    <span className="font-label-caps text-[10px] bg-[#003d9b] text-[#ffffff] px-1.5 py-0.5 rounded font-bold">
                      STOP {activeMapStop.id}
                    </span>
                  </div>
                  <h4 className="font-body-md font-bold text-[#191b23] text-sm leading-snug">
                    {activeMapStop.name}
                  </h4>
              ...
              <div className="space-y-1.5 my-3 text-xs text-[#434654]">
                  ...
                  return (
                    <div key={ra.routeNumber} className="flex items-center justify-between py-1 border-b border-[#ededf8] last:border-0">
                      <div className="flex items-center gap-1.5 font-medium text-[#191b23]">
                        <span className="bg-[#003d9b] text-[#ffffff] font-label-caps px-1.5 py-0.5 rounded text-[10px] font-bold">
                          {ra.routeNumber}
                        </span>
